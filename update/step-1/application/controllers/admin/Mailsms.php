...
class Mailsms extends Admin_Controller
{
    ...
    public function send_individual()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $send_push     = in_array('push', $sms_mail) ? 1 : 0;
            ...
            $notification_array = array('student' => array(), 'parent' => array());

            foreach ($user_array as $user_key => $user_value) {
                if ($user_value['category'] == "student") {
                    $notification_array['student'][] = $user_value['user_id'];
                }if ($user_value['category'] == "parent") {
                    $notification_array['parent'][] = $user_value['user_id'];
                }

            }
            $notice_array = array();
            foreach ($notification_array as $key => $value) {
                if (!empty($notification_array[$key])) {
                    foreach ($notification_array[$key] as $each_key => $each_value) {
                        if ($key == "student") {
                            $student_array = $this->student_model->get($each_value);
                            if ($student_array['app_key'] != "") {
                                $notice_array[] = $student_array['app_key'];
                            }

                        }elseif ($key == "parent") {
                            $student_array = $this->student_model->get($each_value);
                            if ($student_array['parent_app_key'] != "") {
                                $notice_array[] = $student_array['parent_app_key'];
                            }
            ...
            if (!empty($user_array)) {
                ...
                if ($send_push) {
                    $push_array = array('title' => $message_title,
                        'body'                      => $this->customlib->getLimitChar($message));

                    foreach ($notice_array as $notification_array_key => $notification_array_value) {
                        if ($notice_array[$notification_array_key] != "") {

                            $this->pushnotification->send($notification_array_value, $push_array, "mail_sms");
                        }
                    }
                }
        ...
        if ($this->form_validation->run()) {
                ...
                if ($users_value == "student") {
                        ...
                        foreach ($student_array as $student_key => $student_value) {
                            ...
                            if ($send_push) {
                                if ($student_value['app_key'] != "") {
                                    ...
                                    $notification_array[] = $student_value['app_key'];
                                }
                            }
                    ...
                    if (!empty($parent_array)) {
                        foreach ($parent_array as $parent_key => $parent_value) {
                            if ($send_push) {
                                if ($parent_value['parent_app_key'] != "") {
                                    $notification_array[] = $parent_value['parent_app_key'];
                                }
                            }
            ...
            if (!empty($user_array)) {
                ...
                if ($send_push) {
                    $push_array = array('title' => $message_title,
                        'body'                      => $this->customlib->getLimitChar($message));
                    foreach ($notification_array as $notification_array_key => $notification_array_value) {
                        if ($notification_array[$notification_array_key] != "") {
        ...
        if ($this->form_validation->run()) {
            ...
            foreach ($section as $section_key => $section_value) {
                ...
                if (!empty($userlisting)) {
                    foreach ($userlisting as $userlisting_key => $userlisting_value) {
                        if ($send_push) {
                            if ($userlisting_value['app_key'] != "") {

                                $notification_array[] = $userlisting_value['app_key'];
                            }
                        }
                        $array = array(
